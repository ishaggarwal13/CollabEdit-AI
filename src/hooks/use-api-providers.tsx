...
export function useApiProviders() {
  ...
  if (!context) {
    throw new Error(
      "useApiProviders must be used within an ApiProviderProvider"
    );
  }
  return context;
}
...
export function ApiProviderProvider({ children }: { children: ReactNode }) {
        ...
        if (savedProvidersJSON) {
          ...
          const mergedProviders = defaultProviders.map((dp) => {
            const saved = savedProviders.find(
              (sp: ApiProvider) => sp.id === dp.id
            );
            // Always force default providers to stay enabled
            const merged = saved ? { ...dp, ...saved } : dp;
            return { ...merged, enabled: true };
          });

          const customProviders = savedProviders.filter(
            (sp: ApiProvider) => !defaultProviders.some((dp) => dp.id === sp.id)
          );
  ...
  return (
    <ApiProviderContext.Provider
      value={{ providers, setProviders, getProviderById }}
    >
      {children}
    </ApiProviderContext.Provider>
  );
