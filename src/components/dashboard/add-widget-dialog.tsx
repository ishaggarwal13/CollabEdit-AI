...
export default function AddWidgetDialog({
  ...
  const [config, setConfig] = useState<WidgetConfig>({ refreshInterval: "60" });
  ...
  const handleSaveWidget = () => {
    ...
    const newLayout = findOpenPosition(
      currentLayout,
      widgetInfo.defaultLayout.w,
      widgetInfo.defaultLayout.h
    );
    ...
    >
      <DialogContent className="sm:max-w-[650px] max-h-[90vh] flex flex-col">
        <DialogHeader className="shrink-0">
          <DialogTitle>
            {showConfig ? "Configure Widget" : "Add a New Widget"}
          </DialogTitle>
          <DialogDescription>
            {showConfig
