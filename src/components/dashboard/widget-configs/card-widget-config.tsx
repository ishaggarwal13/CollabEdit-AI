...
export default function CardWidgetConfig({ config, setConfig }: CardWidgetConfigProps) {
    ...
    useEffect(() => {
        ...
        if (!url && config.cardType !== 'watchlist' && config.cardType !== 'forex') {
            const selectedProvider = providers.find(p => p.id === config.apiProvider);
            if (selectedProvider) {
                const endpoint = selectedProvider.endpoints.find(e => e.name === 'quote');
                if (endpoint) {
        ...
        setEndpointUrl(url);
    }, [config.apiProvider, config.symbol, config.customApiEndpoint, providers, config.cardType]);
  ...
  return (
    <>
      <div>
        ...
        <Select onValueChange={handleSelectChange('cardType')} value={config.cardType}>
          <SelectTrigger>
            <SelectValue placeholder="Select card type" />
          </SelectTrigger>
          <SelectContent>
            <SelectItem value="watchlist">Watchlist (Pre-defined)</SelectItem>
            <SelectItem value="forex">Currency Exchange (Pre-defined)</SelectItem>
            <SelectItem value="financial-data">Financial Data (Custom)</SelectItem>
          </SelectContent>
        </Select>
      </div>
       ...
       {config.cardType === 'watchlist' ? (
          <>
            <p className="text-sm text-muted-foreground p-4 text-center bg-muted rounded-md">
                The Watchlist card is pre-configured to work with the Quote endpoint of the selected provider.
            </p>
            <div>
          ...
          <>
            <p className="text-sm text-muted-foreground p-4 text-center bg-muted rounded-md">
                This card is pre-configured to work with the Forex endpoint of the selected provider.
            </p>
            <div>
              <Label>API Provider</Label>
              <Select onValueChange={handleSelectChange('apiProvider')} value={config.apiProvider}>
                ...
                <SelectContent>
                  {providers.filter(p => p.enabled && p.endpoints.some(e => e.name ==='forex')).map(provider => (
                      <SelectItem key={provider.id} value={provider.id}>
                          {provider.name}
                      </SelectItem>
                  ))}
        ...
        <div className="space-y-4">
             <div>
                ...
                <p className="text-xs text-muted-foreground mt-1">
                    Select a provider to use its default endpoint, or enter a full URL below.
                </p>
            </div>
            <div>
             ...
             <div>
                <Label htmlFor="array-data-path">Root Path for Data (Optional)</Label>
                <Input
                    id="array-data-path"
                    placeholder="e.g., data.results"
                    value={config.arrayDataPath || ''}
                    onChange={(e) => setConfig({...config, arrayDataPath: e.target.value})}
                />
                <p className="text-xs text-muted-foreground mt-1">
                    If your data is nested inside an object, specify the path to it here.
                </p>
            </div>
