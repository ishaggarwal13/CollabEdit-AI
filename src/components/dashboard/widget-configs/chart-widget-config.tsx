...
export default function ChartWidgetConfig({
  config,
  setConfig,
  ...
  const handleEndpointChange = (value: string) => {
    // Removing support for custom endpoint input; rely on provider endpoints
  ...
  useEffect(() => {
    ...
    setEndpointUrl(url);
  }, [config.apiProvider, config.symbol, config.timeInterval, providers]);
  ...
  return (
    <>
      ...
      <div>
        ...
        <Select
          onValueChange={(value) =>
            setConfig({ ...config, timeInterval: value })
          }
        ...
        <div className="space-y-4">
          ...
          {config.selectedFields && config.selectedFields.length > 0 && (
            <p className="text-xs text-muted-foreground p-2 text-center bg-muted rounded-md">
              Chart widgets will use the selected field to find an array of
              data. It will look for sibling keys named `open`, `high`, `low`,
              `close`, and a date/time field for the axes.
            </p>
          )}
        </div>
      )}
