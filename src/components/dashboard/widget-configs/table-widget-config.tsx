...
export default function TableWidgetConfig({
  config,
  setConfig,
}: TableWidgetConfigProps) {
  const { providers } = useApiProviders();
  const [manualFields, setManualFields] = useState(
    (config.selectedFields || []).map((f) => f.path).join(", ")
  );
  ...
  const handleEndpointChange = (value: string) => {
    // Removing custom endpoint usage; rely on provider 'search' endpoints
    return;
  };

  const handleManualFieldsBlur = () => {
    const fieldNames = manualFields
      .split(",")
      .map((s) => s.trim())
      .filter(Boolean);
  ...
  // Ensure default: first option is pre-defined (stock-search) and default provider Alpha Vantage
  useEffect(() => {
    if (!config.dataSource) {
      setConfig({ ...config, dataSource: "stock-search" });
      return; // subsequent effect will set provider
    }
    if (config.dataSource === "stock-search" && !config.apiProvider) {
      const alpha = providers.find(
        (p) => p.id === "alpha-vantage" && p.enabled
      );
      if (alpha) {
        setConfig({ ...config, apiProvider: alpha.id });
      }
    }
  }, [config.dataSource, config.apiProvider, providers]);
  ...
  return (
    <>
      <div>
        ...
        <Select
          onValueChange={handleSelectChange("dataSource")}
        ...
        >
          ...
          <SelectContent>
            <SelectItem value="stock-search">
              Stock Search (Pre-defined)
            </SelectItem>
      ...
      {config.dataSource === "stock-search" ? (
        <>
          <p className="text-sm text-muted-foreground p-4 text-center bg-muted rounded-md">
            The Stock Search table is pre-configured to work with the Search
        ...
        <>
          ...
          <div>
            <Label htmlFor="array-data-path">
              Root Path for Array Data (Optional)
            </Label>
            <Input
              id="array-data-path"
            ...
            />
            <p className="text-xs text-muted-foreground mt-1">
              If your data is nested inside an object, specify the path to the
              array here.
            </p>
          </div>
          <div>
            <Label htmlFor="display-fields">
            ...
            />
            <p className="text-xs text-muted-foreground mt-1">
              Comma-separated list of fields from the API response to show as
              columns.
            </p>
          </div>
        </>
